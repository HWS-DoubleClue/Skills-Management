...
public class SkillsEmailLogic {
	...
	private List<DcemUser> getRecipientsForRequests() throws Exception {
		...
		String recipientsString = skillsModule.getModulePreferences().getRecipientsForRequestSkills();
		if (recipientsString != null) {
			String[] recipientsNames = recipientsString.split(";");
			for (String loginId : recipientsNames) {
				DcemUser recipient = userLogic.getDistinctUser(loginId.trim());
				if (recipient != null) {
	...
	public void notifySkillsChange(SkillsUserEntity oldSkillsUserEntity, SkillsUserEntity skillsUserEntity) {
		...
		try {
			...
			if (skillsModule.getModulePreferences().isNotificationForHod() == true && hasHeadOf(skillsUserEntity.getDcemUser())
				...
				recipients.add(skillsUserEntity.getDcemUser().getDepartment().getHeadOf());
			}
			if (skillsUserEntity.getReportsTo() != null && skillsUserEntity.getReportsTo().equals(operatorSessionBean.getDcemUser()) == false
					&& recipients.contains(skillsUserEntity.getReportsTo()) == false) {
				recipients.add(skillsUserEntity.getReportsTo());
			}
			if (skillsUserEntity.getDcemUser().equals(operatorSessionBean.getDcemUser()) == false
					&& recipients.contains(skillsUserEntity.getDcemUser()) == false) {
				recipients.add(skillsUserEntity.getDcemUser());
			}
