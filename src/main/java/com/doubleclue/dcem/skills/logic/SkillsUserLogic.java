...
public class SkillsUserLogic {
	...
	@DcemTransactional
	public void addSkillsUser(SkillsUserEntity skillsUserEntity, DcemAction dcemAction) { // TODO Auditing
		skillsUserEntity.setId(skillsUserEntity.getDcemUser().getId());
		em.persist(skillsUserEntity);
	}

	@DcemTransactional
	public void updateUserSkillEntity(SkillsUserEntity skillsUserEntity) throws Exception {
		List<SkillsUserJobProfileEntity> jobPrifiles;
		if (Persistence.getPersistenceUtil().isLoaded(skillsUserEntity, SkillsUserEntity_.userJobProfiles.getName()) == false) {
			SkillsUserEntity skillsUserEntityPrev = getSkillsUserGraphById(skillsUserEntity.getId());
			jobPrifiles = skillsUserEntityPrev.getUserJobProfiles();
		} else {
			jobPrifiles = skillsUserEntity.getUserJobProfiles();
		}
		skillsUserEntity = em.merge(skillsUserEntity);
		for (SkillsUserJobProfileEntity userJobProfile : jobPrifiles) {
			userJobProfile.setMatch(computeMatch(skillsUserEntity, jobProfileEntityLogic.findJobProfileById(userJobProfile.getJobProfile().getId())));
		  }
	...
	@DcemTransactional
	public void deleteSkillsLevel(List<SkillsLevelEntity> skillsLevelEntities) {
		for (SkillsLevelEntity skillsLevelEntity : skillsLevelEntities) {
			if (skillsLevelEntity.getId() == null) {
				return;
			}
			SkillsLevelEntity entity = em.find(SkillsLevelEntity.class, skillsLevelEntity.getId());
			if (entity != null) {
				em.remove(entity);
			}
		}
	...
	public SkillsUserEntity getSkillsUserGraphById(Integer id) throws Exception {
		EntityGraph<?> entityGraph = em.getEntityGraph(SkillsUserEntity.GRAPH_DCEMUSEREXT);
		Map<String, Object> properties = new HashMap<>();
		properties.put("javax.persistence.fetchgraph", entityGraph);
		properties.put("org.hibernate.cacheable", false);
		properties.put("javax.persistence.cache.retrieveMode", CacheRetrieveMode.BYPASS);
		return em.find(SkillsUserEntity.class, id, properties);
	}
	...
	// public List<Tuple> getUserAndSkillsForSkillSearch(List<String> skillsNames, int ordinalLevel, LogicalConjunction logicalConjunction, boolean
	// onlyOwnedSkills)
	// throws Exception {
	...
	// criteriaQuery.multiselect(skillsUserJoin.alias("user"),
	// skillsJoin.get(SkillsEntity_.name).alias("skill"),
	// userSkillRoot.get(SkillsUserSkillEntity_.level).alias("level"),
	// userSkillRoot.get(SkillsUserSkillEntity_.status).alias("skillsStatus"));
	//
	...
	//
	// Predicate userSkillPredicate = logicalConjunction == LogicalConjunction.AND ? cb.and(userHasSkillPredicate.toArray(new
	// Predicate[skillNamePredicates.size()]))
