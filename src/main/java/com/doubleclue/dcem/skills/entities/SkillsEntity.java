...
@NamedQueries({ @NamedQuery(name = SkillsEntity.GET_ALL, query = "SELECT tt FROM SkillsEntity tt ORDER BY tt.name ASC"),
		...
		@NamedQuery(name = SkillsEntity.GET_APPROVED_SKILLS, query = "SELECT tt FROM SkillsEntity tt WHERE tt.status = com.doubleclue.dcem.skills.entities.enums.ApprovalStatus.APPROVED ORDER BY tt.name ASC"), })
